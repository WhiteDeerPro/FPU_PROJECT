...
module tb_fpu_add;
  ...
  assign dbg_lhs_sign          = dbg_is_addsub ? dut.lhs.sign : 1'b0;
  assign dbg_rhs_sign          = dbg_is_addsub ? dut.rhs.sign : 1'b0;
  assign dbg_big_exp           = dbg_is_addsub ? dut.big_op.exp : 11'd0;
  assign dbg_small_exp         = dbg_is_addsub ? dut.small_op.exp : 11'd0;
  assign dbg_exp_diff          = dbg_is_addsub ? dut.exp_diff : 11'd0;
  assign dbg_big_sig           = dbg_is_addsub ? {11'd0, dut.big_op.sig} : 64'd0;
  assign dbg_small_sig_shifted = dbg_is_s ? {15'd0, dut.small_shifted_s} :
                                 dbg_is_d ? {8'd0, dut.small_shifted_d} : 64'd0;
  assign dbg_add_sig_sum       = dbg_is_s ? {15'd0, dut.add_sum_s} :
                                 dbg_is_d ? {8'd0, dut.add_sum_d} : 65'd0;
  assign dbg_sub_sig_diff      = dbg_is_s ? {15'd0, dut.sub_diff_s} :
                                 dbg_is_d ? {8'd0, dut.sub_diff_d} : 64'd0;
  assign dbg_sub_lop_pos       = dbg_is_s ? {1'b0, dut.sub_lop_pos_s} :
                                 dbg_is_d ? dut.sub_lop_pos_d : 7'd0;
  assign dbg_sub_is_zero       = dbg_is_addsub ? dut.sub_is_zero : 1'b0;
  assign dbg_finite_exp        = dbg_is_addsub ? dut.finite_exp : 11'd0;
  assign dbg_finite_sig        = dbg_is_addsub ? dut.finite_sig : 64'd0;
  assign dbg_finite_sign       = dbg_is_addsub ? dut.finite_sign : 1'b0;
  assign dbg_guard             = dbg_is_addsub ? dut.result_grs.guard : 1'b0;
  assign dbg_round             = dbg_is_addsub ? dut.result_grs.round : 1'b0;
  assign dbg_sticky            = dbg_is_addsub ? dut.result_grs.sticky : 1'b0;
  assign dbg_lsb               = dbg_is_addsub ? dut.result_lsb : 1'b0;
  assign dbg_inc               = dbg_is_addsub ? dut.result_round_inc : 1'b0;
  assign dbg_inexact           = dbg_is_addsub ? dut.result_inexact : 1'b0;
  ...
  initial begin
`ifdef DUMP_FSDB
    $fsdbDumpfile("tb_fpu_add.fsdb");
    $fsdbDumpvars("+all");
    $fsdbDumpvars("+struct");
    $fsdbDumpvars("+mda");
    $fsdbDumpvars(0, tb_fpu_add);
`endif
    ...
    $display("tb_fpu_add summary: pass=%0d fail=%0d", pass_cnt, fail_cnt);
    if (fail_cnt != 0) begin
      $fatal(1, "tb_fpu_add failed");
    end

    $finish;
  end

endmodule : tb_fpu_add
