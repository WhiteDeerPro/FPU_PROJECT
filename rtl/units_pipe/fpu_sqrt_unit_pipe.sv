//============================================================================
// Interleaved square-root unit using an rsqrt seed and one private FMA pipe.
//
// Finite path:
//   a  = normalized significand in [1, 2), or 2*a when input exponent is odd
//   x0 = rsqrt_seed(a)
//   t  = a*x
//   e  = 1.0 - t*x
//   x' = x + (0.5*x)*e
...
//   r  = a - q0*q0
//   q  = q0 + (0.5*r)*x
//   result_exp = floor(exponent(input) / 2) + exponent(q)
//============================================================================

module fpu_sqrt_unit_pipe
  import fpu_pkg::*;
(
  input  logic      clk_i,
  input  logic      rst_ni,
  ...
  fp_sqrt_operand_t src;
  logic [52:0]      src_lop_data;
  logic [5:0]       src_lop_pos;
  logic             src_lop_zero;
  logic             op_is_sqrt;
  logic             fmt_is_valid;
  logic             rm_is_valid_w;
  logic             normal_finite_sqrt;
  ...
  fpu_rsqrt_seed_lut u_seed_lut (
    .sig_hi_i        (src.sig_hi),
    .exp_odd_i       (seed_exp_odd),
  ...
  function automatic logic [52:0] lop_data_53(input fpu_data_t data, input fpu_fmt_e fmt);
    if (fmt == FPU_FMT_S) begin
      return {1'b0, data[22:0], 29'd0};
    end
    return {1'b0, data[51:0]};
  endfunction

  function automatic fp_sqrt_operand_t unpack_operand(
    input fpu_data_t data,
    input fpu_fmt_e  fmt,
    input logic [5:0] lop_pos_i
  );
    fp_sqrt_operand_t op;
    logic [31:0]      bits_s;
    logic [63:0]      bits_d;
    logic [52:0]      sig_raw;
    logic [52:0]      sig_norm;
  ...
  function automatic fpu_data_t canonical_nan(input fpu_fmt_e fmt);
    if (fmt == FPU_FMT_S) return nanbox_s(32'h7fc0_0000);
    return 64'h7ff8_0000_0000_0000;
  endfunction

  function automatic fpu_data_t pack_inf(input fpu_fmt_e fmt, input logic sign);
    if (fmt == FPU_FMT_S) return nanbox_s({sign, 8'hff, 23'd0});
    return {sign, 11'h7ff, 52'd0};
  endfunction

  function automatic fpu_data_t pack_zero(input fpu_fmt_e fmt, input logic sign);
    if (fmt == FPU_FMT_S) return nanbox_s({sign, 31'd0});
    return {sign, 63'd0};
  endfunction

  function automatic fpu_data_t pack_max_finite(input fpu_fmt_e fmt, input logic sign);
    if (fmt == FPU_FMT_S) return nanbox_s({sign, 8'hfe, 23'h7f_ffff});
    return {sign, 11'h7fe, 52'hf_ffff_ffff_ffff};
  endfunction

  function automatic fpu_data_t pack_overflow_result(input fpu_fmt_e fmt, input logic sign, input fpu_rm_e rm);
    fpu_rm_e rm_eff;
    logic    to_inf;
    rm_eff = effective_rm(rm);
    to_inf = (rm_eff == FPU_RM_RNE) || (rm_eff == FPU_RM_RMM) ||
             ((rm_eff == FPU_RM_RUP) && !sign) || ((rm_eff == FPU_RM_RDN) && sign);
    return to_inf ? pack_inf(fmt, sign) : pack_max_finite(fmt, sign);
  endfunction

  function automatic fpu_data_t pack_one(input fpu_fmt_e fmt);
    if (fmt == FPU_FMT_S) return nanbox_s(32'h3f80_0000);
  ...
  function automatic fpu_data_t mantissa_data(
    ...
    if (fmt == FPU_FMT_S) begin
      return nanbox_s({1'b0, (8'd127 + {7'd0, exp_odd}), sig_norm[51:29]});
    end
    return {1'b0, (11'd1023 + {10'd0, exp_odd}), sig_norm[51:0]};
  endfunction
  ...
  function automatic fpu_data_t pack_seed_mant(
    ...
    biased_exp = (fmt == FPU_FMT_S) ? (norm_exp + 16'sd127) : (norm_exp + 16'sd1023);
    if (fmt == FPU_FMT_S) begin
  ...
  function automatic fpu_data_t half_data(input fpu_data_t data, input fpu_fmt_e fmt);
    fpu_data_t out;
    logic [31:0] s;
    logic [63:0] d;
    out = data;
    if (fmt == FPU_FMT_S) begin
      s = data[31:0];
      if (s[30:23] > 8'd1) begin
        s[30:23] = s[30:23] - 8'd1;
      end else if (s[30:23] == 8'd1) begin
        s = {s[31], 8'd0, 1'b1, s[22:1]};
      end else begin
        s = {s[31], 8'd0, 1'b0, s[22:1]};
      end
      out = nanbox_s(s);
    end else begin
      d = data;
      if (d[62:52] > 11'd1) begin
        d[62:52] = d[62:52] - 11'd1;
      end else if (d[62:52] == 11'd1) begin
        d = {d[63], 11'd0, 1'b1, d[51:1]};
      end else begin
        d = {d[63], 11'd0, 1'b0, d[51:1]};
      end
      out = d;
    end
    return out;
  ...
  function automatic logic round_inc(input fpu_rm_e rm, input logic sign, input logic lsb,
  ...
  function automatic fpu_resp_t scale_s_result(input fpu_resp_t mant_resp, input logic signed [15:0] exp_delta,
                                               input fpu_rm_e rm);
    fpu_resp_t          out;
    logic [31:0]        bits;
    logic               sign;
    ...
    rounded   = 25'd0;
    inexact   = 1'b0;
    if (bits[30:23] == 8'hff) return mant_resp;
    if (final_exp >= 16'sd255) begin
      ...
      if ((16'sd1 - final_exp) >= 16'sd32) begin
        ...
        for (int bit_idx = 0; bit_idx < 64; bit_idx++) begin
          if (bit_idx < (16'sd1 - final_exp)) lost = lost || ext[bit_idx];
        end
        main_sig = shifted[26:3];
      end
      inexact = shifted[2] || shifted[1] || shifted[0] || lost;
      rounded = {1'b0, main_sig} + {24'd0, round_inc(rm, sign, main_sig[0], shifted[2], shifted[1], shifted[0] || lost)};
      if (rounded[24]) out.result = nanbox_s({sign, 8'd1, 23'd0});
      else out.result = nanbox_s({sign, 8'd0, rounded[22:0]});
  ...
  function automatic fpu_resp_t scale_d_result(input fpu_resp_t mant_resp, input logic signed [15:0] exp_delta,
                                               input fpu_rm_e rm);
    fpu_resp_t          out;
    logic [63:0]        bits;
    logic               sign;
    ...
    rounded   = 54'd0;
    inexact   = 1'b0;
    if (bits[62:52] == 11'h7ff) return mant_resp;
    if (final_exp >= 16'sd2047) begin
      ...
      inexact = shifted[2] || shifted[1] || shifted[0] || lost;
      rounded = {1'b0, main_sig} + {53'd0, round_inc(rm, sign, main_sig[0], shifted[2], shifted[1], shifted[0] || lost)};
      if (rounded[53]) out.result = {sign, 11'd1, 52'd0};
      else out.result = {sign, 11'd0, rounded[51:0]};
      out.fflags[FPU_FFLAG_NX] = out.fflags[FPU_FFLAG_NX] || inexact;
      out.fflags[FPU_FFLAG_UF] = out.fflags[FPU_FFLAG_UF] || inexact;
    end
    return out;
  ...
  function automatic fpu_resp_t scale_result(input fpu_resp_t mant_resp, input fpu_fmt_e fmt,
                                             input logic signed [15:0] exp_delta, input fpu_rm_e rm);
    if (fmt == FPU_FMT_S) return scale_s_result(mant_resp, exp_delta, rm);
    return scale_d_result(mant_resp, exp_delta, rm);
  endfunction

  function automatic fpu_rm_e final_fma_rm(input fpu_req_t req, input fpu_data_t q_data,
                                           input logic signed [15:0] exp_delta);
    logic signed [15:0] q_exp;
    logic signed [15:0] final_exp_est;
    if (req.rs_fmt == FPU_FMT_S) q_exp = $signed({8'd0, q_data[30:23]});
    else q_exp = $signed({5'd0, q_data[62:52]});
    final_exp_est = q_exp + exp_delta;
    return (final_exp_est <= 16'sd0) ? FPU_RM_RTZ : effective_rm(req.rm);
  endfunction

  function automatic fpu_req_t make_fma_req(input fpu_req_t base_req, input fpu_op_e op,
                                            input fpu_data_t src_a, input fpu_data_t src_b,
                                            input fpu_data_t src_c, input fpu_rm_e rm);
    fpu_req_t req;
    req         = base_req;
    req.op      = op;
    req.rm      = rm;
  ...
  assign src_lop_data = lop_data_53(req_i.src_a, req_i.rs_fmt);
  assign src          = unpack_operand(req_i.src_a, req_i.rs_fmt, src_lop_pos);
  assign op_is_sqrt   = (req_i.op == FPU_OP_SQRT);
  assign fmt_is_valid = fmt_supported(req_i.rs_fmt) && (req_i.rs_fmt == req_i.dst_fmt);
  assign rm_is_valid_w = rm_is_supported(req_i.rm);
  assign seed_exp_odd = src.unbiased_exp[0];
  assign seed_data    = pack_seed_mant(req_i.rs_fmt, seed_norm_mant, seed_norm_exp);

  always_comb begin
    special_resp          = '0;
    special_resp.tag      = req_i.tag;
    special_resp.rd       = req_i.rd;
    special_valid_op      = op_is_sqrt && fmt_is_valid && rm_is_valid_w;
    normal_finite_sqrt    = 1'b0;

    if (!special_valid_op) begin
      special_resp.result = '0;
    end else if (src.is_nan) begin
      special_resp.result = canonical_nan(req_i.rs_fmt);
      special_resp.fflags[FPU_FFLAG_NV] = src.is_snan;
    end else if (src.sign && !src.is_zero) begin
      special_resp.result = canonical_nan(req_i.rs_fmt);
      special_resp.fflags[FPU_FFLAG_NV] = 1'b1;
    end else if (src.is_inf) begin
      special_resp.result = pack_inf(req_i.rs_fmt, 1'b0);
    end else if (src.is_zero) begin
      special_resp.result = pack_zero(req_i.rs_fmt, src.sign);
    end else begin
      normal_finite_sqrt = 1'b1;
    end
  end
  ...
  always_comb begin
    ...
    issue_req   = '0;
    for (int ctx_idx = 0; ctx_idx < NUM_CTX; ctx_idx++) begin
      if (!issue_valid && ctx_active[ctx_idx] && ctx_ready[ctx_idx] && !ctx_done[ctx_idx]) begin
        issue_valid = 1'b1;
        issue_ctx   = ctx_idx[2:0];
        issue_kind  = ctx_step[ctx_idx];
        unique case (ctx_step[ctx_idx])
          MICRO_T: begin
            issue_req = make_fma_req(ctx_req[ctx_idx], FPU_OP_FMADD,
                                     ctx_a_mant[ctx_idx], ctx_x[ctx_idx],
                                     pack_zero_pos(ctx_req[ctx_idx].rs_fmt), FPU_RM_RNE);
          end
          MICRO_E: begin
            issue_req = make_fma_req(ctx_req[ctx_idx], FPU_OP_FNMSUB,
                                     ctx_t[ctx_idx], ctx_x[ctx_idx],
                                     pack_one(ctx_req[ctx_idx].rs_fmt), FPU_RM_RNE);
          end
          MICRO_X: begin
            issue_req = make_fma_req(ctx_req[ctx_idx], FPU_OP_FMADD,
                                     half_data(ctx_x[ctx_idx], ctx_req[ctx_idx].rs_fmt),
                                     ctx_e[ctx_idx], ctx_x[ctx_idx], FPU_RM_RNE);
          end
          MICRO_Q: begin
            issue_req = make_fma_req(ctx_req[ctx_idx], FPU_OP_FMADD,
                                     ctx_a_mant[ctx_idx], ctx_x[ctx_idx],
          ...
          MICRO_R: begin
            issue_req = make_fma_req(ctx_req[ctx_idx], FPU_OP_FNMSUB,
                                     ctx_q[ctx_idx], ctx_q[ctx_idx],
                                     ctx_a_mant[ctx_idx], FPU_RM_RNE);
          end
          default: begin
            issue_req = make_fma_req(ctx_req[ctx_idx], FPU_OP_FMADD,
                                     half_data(ctx_r[ctx_idx], ctx_req[ctx_idx].rs_fmt),
                                     ctx_x[ctx_idx], ctx_q[ctx_idx],
                                     final_fma_rm(ctx_req[ctx_idx], ctx_q[ctx_idx], ctx_exp_delta[ctx_idx]));
          end
        endcase
      end
    end
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      for (int pipe_idx = 0; pipe_idx < FMA_LATENCY; pipe_idx++) begin
        ...
        ret_kind_pipe[pipe_idx]  <= MICRO_T;
      end
    end else begin
      valid_q    <= 1'b0;
      valid_op_q <= 1'b0;
      ...
      if (issue_valid) ctx_ready[issue_ctx] <= 1'b0;

      if (fma_valid_o && ret_valid_pipe[FMA_LATENCY-1]) begin
        unique case (ret_kind_pipe[FMA_LATENCY-1])
          MICRO_T: begin
            ctx_t[ret_ctx_pipe[FMA_LATENCY-1]]     <= fma_resp_o.result;
          ...
          MICRO_X: begin
            ...
            if ((ctx_iter[ret_ctx_pipe[FMA_LATENCY-1]] + 3'd1) >= ctx_iter_limit[ret_ctx_pipe[FMA_LATENCY-1]]) begin
              ctx_step[ret_ctx_pipe[FMA_LATENCY-1]] <= MICRO_Q;
            end else begin
              ctx_iter[ret_ctx_pipe[FMA_LATENCY-1]] <= ctx_iter[ret_ctx_pipe[FMA_LATENCY-1]] + 3'd1;
              ctx_step[ret_ctx_pipe[FMA_LATENCY-1]] <= MICRO_T;
            end
            ctx_ready[ret_ctx_pipe[FMA_LATENCY-1]] <= fma_valid_op_o;
          end
          MICRO_Q: begin
      ...
      if (accept_req) begin
        ...
        ctx_done[free_idx]     <= !normal_finite_sqrt;
        ctx_valid_op[free_idx] <= special_valid_op;
        ctx_ready[free_idx]    <= normal_finite_sqrt;
        ctx_step[free_idx]     <= MICRO_T;
        if (normal_finite_sqrt) begin
          ctx_a_mant[free_idx]     <= mantissa_data(req_i.rs_fmt, src.sig_norm, src.unbiased_exp[0]);
