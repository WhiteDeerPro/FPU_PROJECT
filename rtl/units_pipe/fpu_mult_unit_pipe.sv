//============================================================================
// Pipelined floating-point multiply unit.
//
// Flow:
//   req_i
//     -> unpack/classify lhs and rhs, compute result sign
//     -> 53-bit significand multiply through fpu_mult_pipe
//     -> register product and delayed sideband
...
//
// This unit uses the two internal pipeline registers in the common multiplier,
// then adds one unit-level register for the product and request/control
// sideband.  The result is four combinational stages and three-cycle latency.
//============================================================================

module fpu_mult_unit_pipe
  import fpu_pkg::*;
(
  input  logic      clk_i,
  input  logic      rst_ni,
  ...
  logic             s0_valid_op;
  logic             s0_finite_sign;
  logic signed [13:0] s0_base_exp;

  fpu_req_t         s1_req;
  fp_mult_operand_t s1_lhs;
  fp_mult_operand_t s1_rhs;
  logic             s1_valid_op;
  logic             s1_finite_sign;
  logic signed [13:0] s1_base_exp;

  fpu_req_t         s2_req;
  fp_mult_operand_t s2_lhs;
  fp_mult_operand_t s2_rhs;
  logic             s2_valid;
  logic             s2_valid_op;
  logic             s2_finite_sign;
  logic signed [13:0] s2_base_exp;

  fpu_req_t         s3_req;
  fp_mult_operand_t s3_lhs;
  fp_mult_operand_t s3_rhs;
  logic             s3_valid;
  logic             s3_valid_op;
  logic             s3_finite_sign;
  logic signed [13:0] s3_base_exp;
  ...
  // --------------------------------------------------------------------------
  // Stage 0: unpack, operation decode, and product setup
  // --------------------------------------------------------------------------

  assign lhs            = unpack_operand(req_i.src_a, req_i.rs_fmt);
  assign rhs            = unpack_operand(req_i.src_b, req_i.rs_fmt);
  assign s0_finite_sign = lhs.sign ^ rhs.sign;
  assign s0_base_exp    = $signed({3'd0, lhs.exp}) + $signed({3'd0, rhs.exp}) -
                          ((req_i.rs_fmt == FPU_FMT_S) ? 14'sd127 : 14'sd1023) -
                          14'sd104;
  assign s0_valid_op    = (req_i.op == FPU_OP_MUL) &&
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      s1_finite_sign <= s0_finite_sign;
      s1_base_exp    <= s0_base_exp;
      s2_req         <= s1_req;
      s2_lhs         <= s1_lhs;
      s2_rhs         <= s1_rhs;
      s2_valid_op    <= s1_valid_op;
      s2_finite_sign <= s1_finite_sign;
      s2_base_exp    <= s1_base_exp;
      s3_req         <= s2_req;
      s3_lhs         <= s2_lhs;
      s3_rhs         <= s2_rhs;
      s3_valid       <= s2_valid;
      s3_valid_op    <= s2_valid_op;
      s3_finite_sign <= s2_finite_sign;
      s3_base_exp    <= s2_base_exp;
      s3_product     <= product_w;
    end
  end

  // --------------------------------------------------------------------------
  // Stage 3: product leading-one detect and normalization
  // --------------------------------------------------------------------------
  ...
  assign norm_sig        = product_zero ? 64'd0 :
                                          normalize_product_sig(s3_product, product_lop_pos);

  // --------------------------------------------------------------------------
  // Stage 3: exponent calculation and subnormal adjustment
  // --------------------------------------------------------------------------
  ...
  always_comb begin
    ...
    if (!product_zero) begin
      raw_exp = s3_base_exp + $signed({7'd0, product_lop_pos});
  ...
  // --------------------------------------------------------------------------
  // Stage 3: GRS selection and round increment
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  // Stage 3: finite pack and final special-case result mux
  // --------------------------------------------------------------------------
  ...
  always_comb begin
    resp_d     = '0;
    resp_d.tag = s3_req.tag;
    resp_d.rd  = s3_req.rd;
    valid_op_o = s3_valid && s3_valid_op;

    if (!s3_valid || !s3_valid_op) begin
      resp_d = '0;
      resp_d.tag = s3_req.tag;
      resp_d.rd  = s3_req.rd;
    end else if (s3_lhs.is_nan || s3_rhs.is_nan) begin
      resp_d.result = canonical_nan(s3_req.rs_fmt);
      resp_d.fflags[FPU_FFLAG_NV] = s3_lhs.is_snan || s3_rhs.is_snan;
    end else if ((s3_lhs.is_inf && s3_rhs.is_zero) ||
                 (s3_lhs.is_zero && s3_rhs.is_inf)) begin
      resp_d.result = canonical_nan(s3_req.rs_fmt);
      resp_d.fflags[FPU_FFLAG_NV] = 1'b1;
    end else if (s3_lhs.is_inf || s3_rhs.is_inf) begin
      resp_d.result = pack_inf(s3_req.rs_fmt, s3_finite_sign);
    end else if (s3_lhs.is_zero || s3_rhs.is_zero) begin
      resp_d.result = pack_zero(s3_req.rs_fmt, s3_finite_sign);
    end else begin
