...
// Flow:
//   req_i
//     -> decode FMADD/FMSUB/FNMSUB/FNMADD sign controls
//     -> unpack/classify lhs, rhs, and addend
//     -> multiply lhs/rhs significands
//     -> find product/addend top bit positions
//     -> align product and addend into the accumulator domain
//     -> add same-sign magnitudes or subtract opposite-sign magnitudes
//     -> leading-one detect and normalize the finite accumulator result
//     -> handle subnormal right-shift/jam
...
(
  ...
  // --------------------------------------------------------------------------
  // Operation decode, unpack, and sign control
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  // Significand multiply and input leading-one detection
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  // Accumulator exponent selection and operand alignment
  // --------------------------------------------------------------------------
  ...
  always_comb begin
    ...
    if (!product_zero) begin
      prod_bit_base_exp = $signed({5'd0, lhs.exp}) +
                          $signed({5'd0, rhs.exp}) -
                          $signed(exp_bias(req_i.rs_fmt)) -
                          $signed(exp_bias(req_i.rs_fmt)) -
                          16'sd104;
    ...
    if (!addend_sig_zero) begin
      add_bit_base_exp = $signed({5'd0, addend.exp}) -
                         $signed(exp_bias(req_i.rs_fmt)) -
                         16'sd52;
    ...
    product_acc_mag = product_zero ? '0 :
                      align_u106_to_acc(product, prod_bit_base_exp, acc_lsb_exp);
    addend_acc_mag  = addend_sig_zero ? '0 :
                      align_u53_to_acc(addend.sig, add_bit_base_exp, acc_lsb_exp);
  end

  // --------------------------------------------------------------------------
  // Accumulator add/sub and finite sign selection
  // --------------------------------------------------------------------------

  always_comb begin
    acc_sum_ext = '0;
    finite_mag  = '0;
    finite_sign = product_sign;

    if (product_sign == addend_sign) begin
      ...
      finite_sign = addend_sign;
    end else begin
      finite_mag  = '0;
      finite_sign = zero_result_sign(product_sign, addend_sign, req_i.rm);
    end
  end

  // --------------------------------------------------------------------------
  // Finite result leading-one detect, normalization, and exponent adjustment
  // --------------------------------------------------------------------------

  fpu_lop #(
    .DATA_W(64)
  ) u_finite_hi_lop (
    .data_i(finite_mag[159:96]),
  ...
  assign finite_top_exp = acc_lsb_exp + $signed({8'd0, finite_lop_pos});
  assign norm_sig       = finite_zero ? 64'd0 :
                                        normalize_acc_sig(finite_mag, finite_lop_pos);
  ...
  always_comb begin
    ...
    if (!finite_zero) begin
      raw_exp = finite_top_exp + $signed(exp_bias(req_i.rs_fmt));
  ...
  always_comb begin
    ...
    invalid_mul     = (lhs.is_inf && rhs.is_zero) ||
                      (lhs.is_zero && rhs.is_inf);
    product_is_inf  = lhs.is_inf || rhs.is_inf;
    invalid_inf_add = product_is_inf && addend.is_inf &&
                      (product_sign != addend_sign);

    resp_d     = '0;
    resp_d.tag = req_i.tag;
    resp_d.rd  = req_i.rd;
    valid_op_o = op_is_fma &&
    ...
    if (!valid_op_o) begin
      ...
      resp_d.tag = req_i.tag;
      resp_d.rd  = req_i.rd;
    end else if (lhs.is_nan || rhs.is_nan || addend.is_nan ||
      ...
      resp_d.fflags[FPU_FFLAG_NV] = lhs.is_snan || rhs.is_snan ||
                                    addend.is_snan || invalid_mul ||
                                    invalid_inf_add;
    end else if (product_is_inf) begin
      resp_d.result = pack_inf(req_i.rs_fmt, product_sign);
    end else if (addend.is_inf) begin
      resp_d.result = pack_inf(req_i.rs_fmt, addend_sign);
